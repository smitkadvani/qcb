...
def main():
  ...
  parser.add_option('--gres', dest='gres',
      default='gpu',
  ...
  for pi in range(options.processes):
    if not options.restart or not job_completed(options, pi):
      if options.cpu:
        ...
        cmd += 'conda activate enformer-py38;'
      ...
      cmd += ' ${SLURM_SUBMIT_DIR}/enformer_simple_pred.py %s %s %d' % (
      ...
      j = slurm.Job(cmd, name,
          outf, errf,
          queue=options.queue, gpu=num_gpu,gres=options.gres,
          mem=15000, time=options.time, cpu=options.num_cpus,
...
def collect_h5(file_name, out_dir, num_procs):
  ...
  for key in job0_h5_open.keys():
    if key in ['target_ids', 'target_labels']:
      ...
      num_targets = job0_h5_open[key].shape[2]
      pred_length = job0_h5_open[key].shape[1]
      final_h5_open.create_dataset(key, shape=(num_variants, pred_length, num_targets), dtype=job0_h5_open[key].dtype)
